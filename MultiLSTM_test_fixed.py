...
class MultiScaleDataset(Dataset):
    def __init__(self, scales, window_size, horizon):
        # 修复：计算最大尺度步长，确保y的起点在所有尺度的window之后
        max_scale = max(scale_factors)
        # y的起点应该是最大尺度的window_size，确保不包含未来数据
        y_start_offset = max_scale * window_size
        
        # 以最短的尺度为参考（通常是高频数据）
        min_len = min(len(s) for s in scales)
        # 修复：调整max_i的计算，考虑y_start_offset
        max_i = min_len - y_start_offset - horizon + 1
        ...
        for i in range(max_i):
            ...
            if len(x_win) == len(scales):  # 确保每个尺度都有数据
                ...
                # 修复：y的起点从y_start_offset开始，确保不包含X中的数据
                y_start = i + y_start_offset // scale_factors[0]  # 转换为高频数据的索引
                self.y.append(scales[0][y_start:y_start+horizon])

        self.X = torch.tensor(self.X, dtype=torch.float32)
        self.y = torch.tensor(self.y, dtype=torch.float32)
        
        print(f"数据集创建完成:")
        print(f"- 最大尺度步长: {max_scale}")
        print(f"- Y起点偏移: {y_start_offset}")
        print(f"- 样本数量: {len(self.X)}")
        print(f"- X形状: {self.X.shape}")
        print(f"- Y形状: {self.y.shape}")
...
if len(dataset) > 0:
    ...
    print(f"第一个样本:")
    print(f"- X形状: {sample_x.shape}")
    print(f"- Y形状: {sample_y.shape}")
    print(f"- 尺度1 (step=1) X范围: 0-{window_size-1}")
    print(f"- 尺度2 (step=5) X范围: 0-{(window_size*5)-1} (步长5采样)")
    print(f"- Y起点: {max(scale_factors) * window_size}")
    print(f"- Y范围: {max(scale_factors) * window_size}-{max(scale_factors) * window_size + horizon - 1}")
    print(f"修复成功：Y的起点({max(scale_factors) * window_size})大于最大尺度X的终点({(window_size*max(scale_factors))-1})")